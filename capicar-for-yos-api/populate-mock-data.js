...
async function populateStaff() {
  ...
  const records = mockStaff.map(staff => ({ fields: staff }));
  
  try {
    const createdRecords = await base('Staff').create(records);
...
async function populateTasks() {
  ...
  const records = mockTasks.map(task => ({ fields: task }));
  
  try {
    const createdRecords = await base('Task').create(records);
